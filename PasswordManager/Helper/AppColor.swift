//
//  AppColor.swift
//  PasswordManager
//
//  Created by GWL on 15/07/24.
//

import Foundation
import UIKit
extension UIColor {
    struct AppColor {
        public static var greyColor: UIColor {
            return UIColor.hexColor(hex: "#E8E8E8")
        }
        public static var bgColor: UIColor {
            return UIColor.hexColor(hex: "#F9F9F9")
        }
        public static var blueColor: UIColor {
            return UIColor.hexColor(hex: "#3F7DE3")
        }
        public static var greyTextColor: UIColor {
            return UIColor.hexColor(hex: "#CCCCCC")
        }
        public static var redColor: UIColor {
            return UIColor.hexColor(hex: "#F04646")
        }
    }
    class func hexColor(hex: String) -> UIColor {
        var cString: String = hex.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        if cString.hasPrefix("#") {
            cString.remove(at: cString.startIndex)
        }
        if cString.count != 6 {
            return UIColor.gray
        }
        var rgbValue: UInt64 = 0
        Scanner(string: cString).scanHexInt64(&rgbValue)
        return UIColor(
            red: CGFloat((rgbValue & 0xFF0000) >> 16) / 255.0,
            green: CGFloat((rgbValue & 0x00FF00) >> 8) / 255.0,
            blue: CGFloat(rgbValue & 0x0000FF) / 255.0,
            alpha: CGFloat(1.0)
        )
    }
}
